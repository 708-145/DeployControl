...
# Update appliance configuration file
#
###################################################################
...
def parseargv(argv):
    ...
    parser = argparse.ArgumentParser(
        description="Db2 Analytics Accelerator update configuration."
    )
...
def main(argv):
    ...
    print("*************************************************************")
    print()
    print("  Db2 Analytics Accelerator update configuration")
    print()
    print("*************************************************************")
    try:
        lparAccess, licPath, configfile, verbose = parseargv(argv)
        ...
        # test configuration
        _, json, _ = ssc.validate_configuration(configfile)
        print(json)
        log.debug(json)
        ...
        # apply configuration
        resultCode, json, _ = ssc.accelerator_first_time_setup(configfile, None)
        log.debug(resultCode.status_code)
        print(resultCode.status_code)
        if resultCode.status_code != 200:
            raise Exception(
                "Config update call failed with code" + resultCode.status_code
            )
        log.debug(json)
        status = json["status"]
        log.debug(status)
        ...
        if status != "TRIGGERED":
            ...
            raise Exception("Config update call failed " + status)
        print("-- Config update done --")
