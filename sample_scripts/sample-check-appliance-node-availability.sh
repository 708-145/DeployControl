...
#######################################################
#
# Sample script to check if an Accelerator is available
#
#######################################################
...
if [ -z $IP ]
then
      echo "node $IP does not exist, skipping..."
...
# if wget returns appliance signature then OK
# retry for 5mins with sleeptime of 30s
until [[ 0 -lt `wget --no-check-certificate https://${IP}/ibmapp -qO- | grep "need to enable JavaScript" | wc -l` ]]
do
      if (( RETRYTIME < RETRYINTERVAL ))
      then
            echo "appliance on ${IP} did not come up in time. exiting..."
            exit 1
      else
            echo "appliance on ${IP} is not up yet. Sleeping for ${RETRYINTERVAL} seconds"
            sleep ${RETRYINTERVAL}
      fi
...
echo "LPAR ${IP} is in appliance mode"
exit 0
