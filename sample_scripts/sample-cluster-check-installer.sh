...
echo $MGMTIP
./sample-check-installer.sh $MGMTIP
RESULTSUM=$?
...
then
	...
	echo $DN1IP
        ./sample-check-installer.sh $DN1IP
        (( RESULTSUM += $? ))

        echo $DN2IP
        ./sample-check-installer.sh $DN2IP
        (( RESULTSUM += $? ))

        echo $DN3IP
        ./sample-check-installer.sh $DN3IP
        (( RESULTSUM += $? ))

        echo $DN4IP
        ./sample-check-installer.sh $DN4IP
        (( RESULTSUM += $? ))

        echo $DN5IP
        ./sample-check-installer.sh $DN5IP
        (( RESULTSUM += $? ))
fi

if [[ 0 -lt $RESULTSUM ]]
then
       echo "${RESULTSUM} data nodes failed to get into installer mode"
       exit 1
fi

echo "check that cluster ${1} is in installer: done"
